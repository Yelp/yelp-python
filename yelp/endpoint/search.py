...
class Search(object):
    ...
    def search(self, location, current_lat=None, current_long=None, **url_params):
        ...
        """
        url_params.update({"location": location})
        if current_lat is not None and current_long is not None:
            url_params["cll"] = self._format_current_lat_long(current_lat, current_long)

        return SearchResponse(self.client._make_request(SEARCH_PATH, url_params))

    def search_by_bounding_box(
        self, sw_latitude, sw_longitude, ne_latitude, ne_longitude, **url_params
    ):
    ...
    def _format_bounds(self, sw_latitude, sw_longitude, ne_latitude, ne_longitude):
        return "{},{}|{},{}".format(
            sw_latitude, sw_longitude, ne_latitude, ne_longitude
        )

    def _format_coordinates(
        self, latitude, longitude, accuracy, altitude, altitude_accuracy
    ):
